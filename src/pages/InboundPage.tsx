...
export default function InboundPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error("Failed to process inbound operation");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="p-8 max-w-2xl">
      <PageHeader title="Inbound" description="Add new items to warehouse inventory" />
      ...
      <form onSubmit={handleSubmit} className="rounded-xl border border-border bg-card p-6 space-y-5">
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full flex items-center justify-center gap-2 px-5 py-2.5 rounded-lg bg-success text-success-foreground text-sm font-medium hover:opacity-90 transition-all duration-200 disabled:opacity-50"
        >
